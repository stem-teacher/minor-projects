...
# This script performs a clean build of the Stage 5 textbook with Gemini
# It deletes existing content, regenerates all chapters, and compiles the LaTeX document

# Change to the stage5-gemini directory
cd /Users/philiphaynes/devel/teaching/projects/minor-projects/prototypes/textbooks/stage5-gemini
...
python generate_gemini_textbooks.py --generate --stage 5 2>&1 | tee -a stage5-gemini/$LOG_FILE

# Check if generation was successful
if [ ${PIPESTATUS[0]} -eq 0 ]; then
  echo "Successfully generated textbook content" | tee -a stage5-gemini/$LOG_FILE
else
  echo "Error during content generation. Check the log for details." | tee -a stage5-gemini/$LOG_FILE
  exit 1
fi
...
source /Users/philiphaynes/devel/teaching/projects/minor-projects/prototypes/textbooks/textbook_env/bin/activate
python generate_gemini_textbooks.py --compile --stage 5 2>&1 | tee -a stage5-gemini/$LOG_FILE

echo "===============================================" | tee -a stage5-gemini/$LOG_FILE
echo "Clean build process completed at $(date)" | tee -a stage5-gemini/$LOG_FILE
echo "Check stage5-gemini/stage5-gemini-textbook.pdf for the output" | tee -a stage5-gemini/$LOG_FILE
echo "===============================================" | tee -a stage5-gemini/$LOG_FILE

# Go back to stage5-gemini directory
cd stage5-gemini
...
echo "The compiled PDF should be available at: stage5-gemini-textbook.pdf"