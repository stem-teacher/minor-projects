#!/usr/bin/env python3
"""
Script to create a clean tufte-book LaTeX file.
"""

import os
import sys

def create_clean_tufte(subject, output_dir):
    """Create a clean LaTeX file using tufte-book class."""
    title = "Chemistry" if subject == "chemistry" else "Physics"
    
    # Create a clean template with tufte-book
    content = f"""% Stage 6 {title} Textbook (Clean Tufte Version)
...
"""
    ...
    if os.path.exists(main_tex):
        with open(main_tex, 'r') as f:
            original = f.read()
        backup_file = f"{output_dir}/main-textbook.tex.original"
    ...
    print(f"Created clean tufte file: {main_tex}")

def main():
    if len(sys.argv) < 2:
        print("Usage: python clean_tufte.py [chemistry|physics|both]")
        sys.exit(1)
    
    subject = sys.argv[1].lower()
    
    if subject == 'chemistry' or subject == 'both':
        create_clean_tufte('chemistry', 'stage6-chemistry')
    
    if subject == 'physics' or subject == 'both':
        create_clean_tufte('physics', 'stage6-physics')
