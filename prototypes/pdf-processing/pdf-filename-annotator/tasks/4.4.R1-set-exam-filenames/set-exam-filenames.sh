...
TASK_ID="4.4.R1-set-exam-filenames"
...
do
    ...
    # Use --in-place (but no backup-suffix since the tool no longer creates backups internally)
    COMMAND="\"$SET_BIN\" --input \"$TARGET_PDF\" --in-place --label \"$LABEL\" --value \"$STUDENT_IDENTIFIER\""
