...
TASK_ID="4.3-copy-template-to-exam"
...
# --- !! Using the freshly created template with annotations instead of final (which has issues) !! ---
SOURCE_PDF="$PROJECT_ROOT/test_resources/output/sample_exam_marking_template_annotated.pdf"
...
# List ALL the labels we added in Task 4.1
LABELS_TO_COPY=(
...
echo "Goal: Copy annotations from template ($SOURCE_PDF) to exam ($TARGET_PDF)." >> "$STEP_LOG"
...
# We omit --target-page, so annotations are copied to the *same page number*
# as they were found on in the source template.
...
if [ $FINAL_CHECK_STATUS -eq 0 ]; then
  ...
  echo "Validation: Successfully ran cp-annotation. Final file: $OUTPUT_PDF." >> "$STEP_LOG"
  echo "Manual Verification Recommended: Please open $OUTPUT_PDF and check that annotations from the template appear correctly positioned on the relevant pages." >> "$STEP_LOG"
  exit 0
else
