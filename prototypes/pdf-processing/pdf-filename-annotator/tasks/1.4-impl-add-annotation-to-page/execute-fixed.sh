...
# --- Log Start ---
echo "## ${CURRENT_DATETIME} - Executing Task ${TASK_ID} (Fixed)" >> "$STEP_LOG"
...
echo "" >> "$STEP_LOG"
echo "---" >> "$COMMAND_LOG"
echo "### ${CURRENT_DATETIME} - Task ${TASK_ID} Execution (Fixed)" >> "$COMMAND_LOG"
echo "" >> "$COMMAND_LOG"

# Get the last line number to remove the previous implementation
echo "[Action 0] Checking for previously added function and removing if present" | tee -a "$STEP_LOG"
LINE_COUNT=$(wc -l < "$UTILS_FILE")
SEARCH_STRING="pub fn add_annotation_to_page"
FOUND_LINE=$(grep -n "$SEARCH_STRING" "$UTILS_FILE" | tail -1 | cut -d: -f1)

if [ -n "$FOUND_LINE" ]; then
  echo "Found add_annotation_to_page function at line $FOUND_LINE, removing it" | tee -a "$STEP_LOG" "$COMMAND_LOG"
  TEMP_FILE="${UTILS_FILE}.tmp"
  head -n $((FOUND_LINE - 1)) "$UTILS_FILE" > "$TEMP_FILE"
  mv "$TEMP_FILE" "$UTILS_FILE"
  echo "Function removed. Line count decreased from $LINE_COUNT to $(wc -l < "$UTILS_FILE")" | tee -a "$STEP_LOG" "$COMMAND_LOG"
fi
