...
fn main() {
    ...
    let template_path = Path::new("../pdf-filename-annotator/requirements/after-marking.pdf");
...
fn extract_annotations(template_path: &Path) -> Result<Vec<PdfAnnotation>, lopdf::Error> {
    ...
    let annotations = match first_page.get(b"Annots") {
        Ok(Object::Array(annots)) => {
            println!("Found annotation array directly");
            annots.clone()
        },
        Ok(Object::Reference(ref_id)) => {
            println!("Found annotation reference: {:?}", ref_id);
            // Try to resolve the reference to get the array
            match doc.get_object(*ref_id) {
                Ok(Object::Array(annots)) => {
                    println!("Successfully resolved reference to annotation array");
                    annots.clone()
                },
                Ok(other) => {
                    println!("Reference resolved to unexpected type: {:?}", other);
                    return Ok(Vec::new());
                },
                Err(e) => {
                    println!("Failed to resolve annotation reference: {:?}", e);
                    return Ok(Vec::new());
                }
            }
        },
        Ok(other) => {
            println!("Annotations found but in unexpected format: {:?}", other);
            return Ok(Vec::new());
        },
        Err(_) => {
            // Try to dump all keys in the page to see what's available
            println!("No 'Annots' key found on the first page. Available keys:");
            for (key, _) in first_page.iter() {
                println!("  {}", String::from_utf8_lossy(key));
            }
            
            // Let's look for content objects instead which might contain annotations
            if let Ok(content) = first_page.get(b"Contents") {
                println!("Page has Contents: {:?}", content);
            }
    ...
    for (index, annot_ref) in annotations.iter().enumerate() {
        match annot_ref {
            Object::Reference(id) => {
                println!("Processing annotation #{} (Reference ID: {:?})", index + 1, id);
...
fn extract_annotation_data(dict: &Dictionary) -> Option<PdfAnnotation> {
    ...
    if let Ok(Object::Dictionary(ap)) = dict.get(b"AP") {
        ...
        if let Ok(appearance_ref) = ap.get(b"N") {
            ...
            properties.insert("normal_appearance".to_string(), format!("{:?}", appearance_ref));
        }
    }
    ...
    for key in [b"T" as &[u8], b"P", b"M", b"CA", b"RC", b"OC", b"StructParent"] {
